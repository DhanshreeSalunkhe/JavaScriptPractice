...
$(document).ready(()=>{
    ...
    const brand = $('.brand');
    const firstIndexBrand = brand.eq(0)
    console.log(firstIndexBrand);
})

function createH2(){
    const h2 = $('<h2 class="heading"> Hello in jquery </h2>')
    $('body').append(h2)
}

const createCard =()=> {
const cardtemp = getCardtemp(productList);
const card = $(cardtemp);   //it create dom element object
$("body").append(card);  //finding the element in dom and return body dom object

}


const getCardtemp =(productInfo)=> {

    const {preview,name,price,brand} = productInfo||{} //way od destructuring
